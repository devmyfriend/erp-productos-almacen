...
const create = async (req, res) => {
	try {
		...
		if (productFound.exist) {
			return res
				.status(409)
				.json({ error: 'El código del Activo ya esta en uso' });
		}

		const newProduct = Object.assign(data, { TipoProductoId: 5 });

		const createProduct = await ProductModel.create(newProduct);

		return res.status(200).json({
			message: 'Se ha creado el Activo',
...
const update = async (req, res) => {
	try {
		...
		if (!productFound.exist) {
			return res.status(404).json({ error: 'No se ha encontrado el Activo' });
		}
		...
		return res.status(200).json({
			message: 'Se ha editado el Activo',
		});
	} catch (error) {
		console.log(error);
		return res.status(500).json({
			error: 'Error al editar el Activo',
		});
	}
};
...
const disable = async (req, res) => {
	...
	return res.status(200).json({ message: 'Activo eliminado' });
};
